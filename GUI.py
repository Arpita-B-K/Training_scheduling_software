...
class main_page_emp(LabelFrame):
	def __init__(self, master):
		...
		Button(self, text="Logout", bg="blue", fg="white", borderwidth=4,
                       relief=GROOVE, command=lambda: master.switch_frame(signin_page), font=('Helvetica 10 bold')).grid(row=15, column=5)
...
class main_page_admin(LabelFrame):
	def __init__(self, master):
		...
		Button(self, text="SUBMIT", font='Helvetic 10 bold', bg="light blue", fg="white").grid(row=4)

		T=Text(self, width=20, height=10)
		T.grid(row=5)
